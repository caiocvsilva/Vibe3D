...
from lib.core.config import VIBE_DB_DIR, CASIAB_DIR
...
def read_data(root_folder, subset, debug=False):
    ...
    identities = sorted([x for x in os.listdir(osp.join(root_folder, 'cropped_frames'))])
    for identity in identities:
        #print(identity)

        idx = int(identity)
        if subset == 'train' and idx > 74 or subset == 'test' and idx <= 74:
            continue

        conditions = sorted([x for x in os.listdir(osp.join(root_folder, 'cropped_frames', identity))])
        for condition in conditions:
            print('\t', condition)
            angles = sorted([x for x in os.listdir(osp.join(root_folder, 'cropped_frames', identity, condition))])
            for angle in angles:
                print('\t\t', angle)
                sys.stdout.flush()

                frames = sorted([x for x in os.listdir(osp.join(root_folder, 'cropped_frames', identity, condition, angle))])
                #if len(frames) < 20:
                #    print(osp.join(root_folder, 'cropped_frames', identity, condition, angle))

                img_paths = []
                sil_paths = []
                batch = []
                for frame in frames:
                    img_path = osp.join(root_folder, 'cropped_frames', identity, condition, angle, frame)
                    img_paths.append(img_path)
                    sil_path = osp.join(root_folder, 'cropped_silhouettes', identity, condition, angle, frame)
                ...
                dataset['vid_name'].append(np.array([identity+'_'+condition+'_'+angle]*len(frames)))
...
if __name__ == '__main__':
    ...
    dataset = read_data(CASIAB_DIR, 'train', debug=debug)
    joblib.dump(dataset, osp.join(VIBE_DB_DIR, 'casiab_train_db.pt'))

    dataset = read_data(CASIAB_DIR, 'test', debug=debug)
