...
def eval(generator, gait_head, data_loader, device, writer=None, epoch=None):
    batch_size = 32
    generator.eval()
    gait_head.eval()
...
def main():
    ...
    # ========= TensorBoard Writer ========= #
    writer = SummaryWriter(log_dir='runs/vibe_gait_experiment')
    ...
    val_eer = eval(generator, gait_head, data_loaders[1], device, writer=writer, epoch=0)

    print('eval in', time.time()-start, 'seconds')
    sys.stdout.flush()

    # Lists to accumulate loss/EER for plotting if desired
    train_losses = []
    val_eers = [val_eer]
    ...
    for epoch in range(cfg.TRAIN.END_EPOCH):
        ...
        for iteration in range(cfg.TRAIN.NUM_ITERS_PER_EPOCH):
            ...
            # Log batch loss
            writer.add_scalar('Loss/train_batch', loss.item(), epoch * cfg.TRAIN.NUM_ITERS_PER_EPOCH + iteration)

        epoch_loss = running_loss / cfg.TRAIN.NUM_ITERS_PER_EPOCH
        ...
        start = time.time()
        val_eer = eval(generator, gait_head, data_loaders[1], device, writer=writer, epoch=epoch + 1)
