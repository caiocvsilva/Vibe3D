...
def eval(generator, gait_head, data_loader, device):
    ...
    with torch.no_grad():
        ...
        for j, batch_img in enumerate(data_loader):
            print('Batch', j, 'of', len(data_loader))
            sys.stdout.flush()
...
def main():
    ...
    for epoch in range(cfg.TRAIN.END_EPOCH):
        ...
        for iteration in range(cfg.TRAIN.NUM_ITERS_PER_EPOCH):
            print('Iteration', iteration, 'of', cfg.TRAIN.NUM_ITERS_PER_EPOCH)
            sys.stdout.flush()
        ...
        torch.save(gait_head.state_dict(), '/blue/sarkar.sudeep/mauricio.segundo/models/new-allrotmat-'+cfg.TRAIN.DATASET+'-gaithead-'+str(epoch+1)+'.pytorch')
        torch.save(generator.state_dict(), '/blue/sarkar.sudeep/mauricio.segundo/models/new-allrotmat-'+cfg.TRAIN.DATASET+'-generator-'+str(epoch+1)+'.pytorch')
